...
- Close R2 with the complete Vetch v13 browser-package rollout and its bounded-contract correction. Clean Vicia source `77a3008` generates one package containing the WASM, JS glue, Rust and TypeScript declarations, manifest, licenses, and provenance; vendor-only Vetch commit `371719f` passes the 78-test real-Chrome format matrix, staged package integration, the complete Vetch authority suite, TypeScript checking, and the production build. Vetch `8877a11` binds cutover acceptance to matching build and integration provenance instead of a duplicated commit literal. The package version, default v12 writer, explicit maintenance ownership, and Vetch scheduling policy remain unchanged. R3 bounded-memory recompact is next.
- Keep persisted current projections live across a bounded resident tail. Exact-watermark single-attribute `count`/`sum` reads rebuild complete touched-entity intervals through the authoritative reducer, cap tail facts and history at 65,536 entries each plus 8 MiB resident bytes, and fall back before installing over-budget state. The clean 1M receipt merges 1,024 post-publication entities exactly and serves cached p50/p95 at 120.685/130.062 ms without full-image decode, measured query RSS growth, or arbitrary-Datalog routing changes.
...
- Add bounded net valid-time diffs to transaction-pinned read views. Native `ReadView::valid_time_diff` compares value visibility at two valid instants under the view's transaction basis and returns only `Appeared`/`Disappeared` rows with one proving window each; interval-internal churn stays on the raw history surface by design (`docs/DIFF_READER_DESIGN.md`). Scope is one attribute plus an optional exact set of up to 128 entities; pages cap at 10,000 rows, 65,536 source entries, and 8 MiB, break only at entity-group boundaries, validate continuations against the exact view/request, and fail closed instead of truncating. Retraction, half-open boundary, backdated-correction, multi-value, layer-merge, pinning, and paging semantics are pinned by an 18-test suite, and the clean 1M receipt measures 0.665 ms p95 for the 128-entity scope and 0.207 ms p95 for the attribute scope (`docs/BENCHMARKS.md`). Datalog, file-format bytes, and existing APIs are unchanged.
- Add bounded exact entity/attribute fact-log paging to transaction-pinned read views. Native `ReadView::entity_attribute_history` and browser `BrowserReadView.entityAttributeHistory()` return full `FactRecord` assertion/retraction identity in deterministic EAVT order, validate opaque continuations against the view and range, and cap each complete page at 10,000 rows, 65,536 source entries, and 8 MiB. Native, in-memory Chrome, and sparse `openPaged()` regressions page 12,005 Vetch canvas root links as 10,000 plus 2,005 rows without loss, duplication, full-image loading, or post-view leakage. Datalog and file-format bytes remain unchanged.
...
- Add explicit maintenance-owned current-projection rebuilds. Native `MaintenanceLedger::rebuild_current_projections()` checkpoints pending WAL work and publishes 1–32 attributes under one v13 catalog generation; browser `BrowserMaintenanceLedger.rebuildCurrentProjections()` computes the same detached page patch, commits it with page 0 in one IndexedDB transaction, and reopens only after success. Both share one captured valid-time floor and bounded 15%-of-ledger image budget, while alternating catalog slots reuse the inactive predecessor arena. Interactive APIs and production query routing remain on the full-history ledger.
- Add explicit v13 current-projection publication authority behind repository-only test and benchmark surfaces. The ordinary writer remains v12; publication appends a checksummed page image and catalog before atomically selecting either of two page-0 generation slots. Reopen requires an exact ledger generation/watermark match, falls back from a corrupt newer copy, and preserves ledger readability when every derived copy is damaged. The clean 1M receipt validates all three temporal probes after reopen without changing production query routing, public APIs, or the Vetch package.
- Add a `vicia.storage-layout.v3` acceptance receipt for the exact v12 fill frontier. The clean rotated 1M matrix retains the historical v2 candidate contract, measures fill 86/87/88/89 alongside 75/85/90/95/100, and selects fill 87: the 276.590 MiB image is 11.93% smaller than fill 75 while checkpoint, point, aggregate, RSS, exact-result, and structural gates all pass. This measurement does not change the production fill or browser package.
- Add a mutation-audited repeated-aggregate retention receipt. Five clean 1M fresh-child pairs compare one and twenty exact current aggregates, attribute RSS before/after allocator trim and database drop, and prove zero median retained growth at 1.125 MiB without changing the library API, allocator, or file format.
- Add a shared 4,096-source bounded relationship-read fixture that distinguishes Vetch's reversible keyword-valued owner relation from true UUID Ref values. Native and real-Chrome `openPaged()` tests prove exact transaction-pinned results, scoped/unscoped retractions, undersized-budget rejection, and sparse page residency across raw Datalog and typed `refsTo()` without adding a public selector or changing file bytes.
- Make the runnable browser example capability-first: foreground atomic writes and bounded reads use `BrowserInteractiveLedger`, while maintenance, verified export, and strict import run in one-operation `BrowserMaintenanceLedger` workers under the same Web Lock. A real-Chrome receipt proves export/import/reopen and zero remaining workers. New migration guidance keeps `Minigraf` and `BrowserDb` supported throughout 1.x and allows 2.0 removal only after exact replacement coverage plus at least twelve months of published notice.
- Add capability-scoped ledger handles for ordinary and lifecycle work. Native `InteractiveLedger` exposes only transact/retract writes, explicit write transactions, bounded pinned read views, and the transaction cursor; its drop leaves WAL-backed work for an explicit `MaintenanceLedger` lifetime, which owns idle maintenance, backup, and full fact-log export. Browser `BrowserInteractiveLedger` and `BrowserMaintenanceLedger` both use the paged IndexedDB constructor and separate atomic writes/bounded reads from maintenance/verified export/strict import. Existing `Minigraf` and `BrowserDb` APIs remain compatible. Rust compile-fail guards, generated TypeScript surface validation, and the real-Chrome suite pin the boundary without changing file bytes or Datalog semantics.
...
- Point-in-time and default Datalog relations now coalesce identical EAV assertions across overlapping valid-time windows before joins. This prevents multiplicative bindings from exhausting `:max-results`, while `:any-valid-time` and full ledger history retain every distinct window.
- Foreground read views now keep the 10,000-row complete-result ceiling separate from the caller's source-work budget, so selective aggregates may inspect more than 10,000 rows without allowing a non-aggregate result above the public bound. Resident projection tails now count empty retract replacements as retained entities, account for keys and B-tree entry ownership, and fall back after 65,536 cumulative replacement entities instead of allowing tombstones to grow without bound. Native and browser paths retain whole-result rejection with no truncation.
- Keep the pending-isolation benchmark valid under generation-time source-work limits by sizing its repository-only query ceiling for the 1.01M selected-control fixture, and validate the current reducer/visitor diagnostic fields instead of rejecting the expanded receipt shape.
- Native file-backed sessions now classify storage failures by whether the current handle remains safe to reuse. Pre-apply WAL rejection and backup destination failures return one `storage` frame and continue; committed-read, lock, post-WAL apply/checkpoint, status, maintenance, export, and backup-source failures return one frame and exit nonzero. Session startup also disables threshold and drop checkpoints so acknowledged writes remain under WAL replay authority until an explicit checkpoint, maintenance, or backup request. The classification wrapper is native-only, preserving browser sparse-page demand errors as typed control flow. Real child-process regressions prove both recoverable continuation and fatal exit followed by exact WAL replay, while the 74-test real-Chrome matrix pins browser paging.
