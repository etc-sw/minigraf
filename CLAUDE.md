...
- **Self-contained** - Minimal dependencies; core engine (`libminigraf.so`) under 1 MiB. The CLI binary and the browser WASM bundle have their own separate budgets — always name the artifact, never say "binary size" unqualified. See the size-budget table in PHILOSOPHY.md.
...
1. **`src/graph/`** — EAV fact store with bi-temporal support
   - `types.rs`: `Fact`, `Value`, `EntityId`, `TxId`, `VALID_TIME_FOREVER`, `RETRACT_ALL_VALID_FROM`
   - `storage.rs`: `FactStorage` — in-memory store, `transact_batch`, `retract_batch`, `get_facts_as_of`, `get_facts_valid_at`, `net_asserted_facts`

2. **`src/storage/`** — Persistence layer
   - `mod.rs`: `StorageBackend` trait, `FileHeader` v9 (84 bytes), `CommittedFactReader` / `CommittedIndexReader` traits
   ...
   - `persistent_facts.rs`: `PersistentFactStorage` — v9 save/load, auto-migration v1–v8→v9
...
Auto-migrates v1/v2/v3/v4/v5/v6/v7/v8 → v9 on open/checkpoint.
